...
## 1: make
make.prosse.multi <- function (tree, traits=NULL, states, states.sd, lambda, control=NULL) {
...
initial.tip.prosse.multi <- function (cache, control, x) {
	...
	init <- function (group2, n.tip, len, nx, npad, states, states.sd, traits=NULL, k=NULL, h=NULL) {
...
check.pars.prosse.multi <- function(b, mu, lambda.x, q, p, drift, diffusion) {
  ...
  if ( b < 0 || mu < 0 || any(lambda.x < 0) || diffusion <= 0 )
